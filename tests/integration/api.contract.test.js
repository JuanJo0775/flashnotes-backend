 ...
 * seg�n API.md
 * 
 * Nota: setup.js maneja la conexi�n a MongoDB autom�ticamente
 */
const request = require('supertest');
const app = require('../../src/app');
const Note = require('../../src/models/Note');
// Helper para incluir la cookie de sesi�n
...
describe('API Contract - Response Format', () => {
    ...
    describe('Respuesta de Error (4xx, 5xx)', () => {
        test('Validaci�n fallida devuelve 400 con error code', async () => {
            const res = await sendWithSession(request(app)
                .post('/api/notes'))
                .send({
        ...
        test('ID inv�lido devuelve 400', async () => {
            const res = await sendWithSession(request(app)
        ...
        test('Restaurar nota que no est� en papelera devuelve 404', async () => {
            const createRes = await sendWithSession(request(app)
                .post('/api/notes'))
                .send({
                    title: 'Active note',
    ...
    describe('Undo/Redo Workflow', () => {
        test('Undo despu�s de m�ltiples cambios restaura versi�n anterior', async () => {
            const createRes = await sendWithSession(request(app)
                .post('/api/notes'))
                .send({
                    title: 'Original',
